...
class LaCTLVSM(nn.Module):
    ...
    def forward(self, input_data_dict, target_data_dict, use_mask=False):
        ...
        with torch.autocast(device_type="cuda", enabled=False), torch.no_grad():
            ...
            P = self.patch_size
            if use_mask:
                # 1=trusted, 0=untrusted (slight dilation)
                M = input_data_dict["mask"].float()
                B,V,_,Hm,Wm = M.shape

                M_hard = (M > 0.5).float()                     # ensure binary (0/1 only)
                pose_ch = len(self.pose_keys) * 3
                rgb = input_data_dict["posed_image"][:, :, pose_ch:pose_ch+3]
                fill = -1.0
                rgb = rgb * M_hard + fill * (1.0 - M_hard)
                input_data_dict["posed_image"][:, :, pose_ch:pose_ch+3] = rgb

                conf_tokens = torch.nn.functional.avg_pool2d(
                    M_hard.reshape(B*V, 1, Hm, Wm), P, stride=P).view(B, V, 1, Hm//P, Wm//P).flatten(3)
                token_conf_inp = conf_tokens.squeeze(2).reshape(B, -1)

                num_img_tokens = (Hm//P) * (Wm//P)
                num_target_tokens = num_target_views * num_img_tokens
                token_conf_tar = torch.ones(B, num_target_tokens, device=token_conf_inp.device)
                
                # If target has a mask, focus confidence only on hole regions
                Mt = target_data_dict["mask"].float()          # (B, V_tar, 1, H, W), 1=known
                Bt, Vt, _, Ht, Wt = Mt.shape

                # Light smoothing for boundary refinement; reuse the input-side helper
                hole_soft = (1.0 - Mt) + 0.2                       # 1=hole (area of interest)

                # Downsample to token scale (AvgPool → per-token hole ratio)
                hole_tok = F.avg_pool2d(
                    hole_soft.view(Bt*Vt, 1, Ht, Wt), P, stride=P
                ).view(Bt, Vt, 1, Ht//P, Wt//P).flatten(3)      # (B, Vt, 1, L_img)

                hole_tok = hole_tok.squeeze(2).reshape(Bt, -1)  # (B, Vt*L_img) == (B, num_target_tokens)

                # Apply “holes-only” weighting to target token confidence
                # - multiply as-is to keep only hole tokens active
                # - optionally soften within holes via pow(alpha)
                token_conf_tar = token_conf_tar * hole_tok.clamp(0.0, 1.0)
                token_conf_tmp = torch.cat([token_conf_inp, token_conf_tar], dim=1)  # (B, L_total)
                token_conf_tmp = token_conf_tmp.clamp_min(1e-3)
        ...
        if token_conf_tmp is not None:
            info["token_conf"] = token_conf_tmp
        ...
        for dd in (input_data_dict, target_data_dict):
            for k in ["ray_o","ray_d","o_cross_d","pose_only","posed_image","normalized_image"]:
                if k in dd: del dd[k]
