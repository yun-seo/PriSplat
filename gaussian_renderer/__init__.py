...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, separate_sh = False, override_color = None, use_trained_exp=False):
    ...
    if use_trained_exp:
        ...
        try:
            ...
            exposure = torch.eye(3, 4, device="cuda") - thr
        rendered_image =  rendered_image * (torch.relu(exposure[2, 3].repeat(3)[:, None, None]+thr)+1)     # yyh : no bias, and use only diagonal elements
        ################################################
