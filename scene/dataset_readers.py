...
def readColmapSceneInfo(path, images, depths, eval, train_test_exp, dataname='wg', llffhold=8):
    try:
        if dataname == 'wg':
    ...
    if dataname == 'wg':
        ...
        with open(os.path.join(path, f"train_list.txt"), 'r') as file:
            train_cam_names_list = [line.strip() for line in file]
